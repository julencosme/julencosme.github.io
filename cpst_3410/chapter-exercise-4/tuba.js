...
function verifyAcres() {
  ...
  try {
    if(!(acresBox.value > 0)) {
      throw "Please enter a number of acres greater than 0."; // throw error in case acres entry is empty
    }
  } catch(message) {
      validity = false; // set validity value to false
      messageText = message; // set error message thrown by try statement
      acresBox.value = ""; // clear erroneous entry from input box
  } finally {
    acresComplete = validity; // set value of global variable acresComplete to value of validity variable (i.e., true/false); determines whether recommendation is ready to be generated
    messageElement.innerHTML = messageText; // set sidebar message to error message text if error thrown; otherwise, set as hitherto specified empty string
